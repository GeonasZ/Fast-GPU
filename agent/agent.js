...
function bandwidthStatus(){const installed=Boolean(nvbandwidthPath);let lastRun=null,results=null;try{const report=JSON.parse(fs.readFileSync('/var/lib/fast-gpu/benchmark.json','utf8'));lastRun=report.generatedAt||null;results=report.nvbandwidth||null}catch{}return{installed,path:nvbandwidthPath,lastRun,results}}
...
async function testReachability(signal){
  ...
  fs.mkdirSync('/var/lib/fast-gpu',{recursive:true});fs.writeFileSync('/var/lib/fast-gpu/reachability.json',JSON.stringify(result,null,2));return result
}
...
async function benchmark(mode='quick',signal){
  ...
  const gpuCount=csvLines(gpu.stdout).length,ncclScript='/tmp/fast-gpu-nccl-check.py';
  ...
  fs.mkdirSync('/data',{recursive:true});const diskFile='/data/.fast-gpu-fio';
