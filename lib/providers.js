...
function redactSensitive(value) {
  ...
  return text
    .replace(/(TAILSCALE_AUTH_KEY(?:\\?["']?\s*[:=]\s*|=))(?:\\?["']?)[^"'\\;\s]+/gi,'$1[REDACTED]')
    .replace(/\btskey-[A-Za-z0-9_-]+\b/g,'[REDACTED]');
}
...
async function call(provider, base, pathname, { method = 'GET', headers = {}, body, query } = {}) {
  ...
  try {
    ...
    data=JSON.parse(redactSensitive(data));
...
class HyperstackAdapter {
  ...
  async listInstances(){const r=await call(this.name,this.base,'core/virtual-machines',{headers:this.headers()});if(r.status===false)throw new ProviderError(this.name,r.message||'获取实例失败',502,r);return(r.instances||[]).map(x=>({id:String(x.id),provider:'hyperstack',providerName:this.name,name:x.name,status:normalizeStatus(x.status),gpu:x.flavor?.gpu,gpuCount:Number(x.flavor?.gpu_count||1),region:x.environment?.region,image:x.image?.name,price:null,priceUnit:'USD/hour',ip:null,publicIp:false,accessType:'tailscale',sshUser:this.env.HYPERSTACK_IMAGE_USER,cudaProfile:'自动检测中',raw:x}));}
  async create(o){const environment=this.environmentFor(o.region);const keyName=this.env.HYPERSTACK_KEY_NAME,imageName=this.env.HYPERSTACK_IMAGE_NAME,imageUser=this.env.HYPERSTACK_IMAGE_USER;if(!environment||!keyName||!imageName||!imageUser)throw new ProviderError(this.name,'运行商配置未',503);if(!this.env.TAILSCALE_AUTH_KEY)throw Object.assign(new ProviderError(this.name,'缺少服务端环境变量 TAILSCALE_AUTH_KEY，未创建 VM',503),{code:'tailscale_auth_key_missing'});if(!this.env.FLEET_BOOTSTRAP_URL)throw new ProviderError(this.name,'缺少 FLEET_BOOTSTRAP_URL，无法自动装机',503);const startup=hyperstackStartup(this.env,o);const body={count:1,environment_name:environment,flavor_name:o.productId,key_name:keyName,name:o.name||'gpu-fleet',image_name:imageName,assign_floating_ip:false,user_data:startup.userData};const r=await call(this.name,this.base,'core/virtual-machines',{method:'POST',headers:this.headers(),body});if(r.status===false)throw new ProviderError(this.name,r.message||'创建 VM 失败',502);const instance=r.instances?.[0];return{id:String(instance?.id),provider:'hyperstack',name:instance?.name||body.name,status:'provisioning',image:body.image_name,publicIp:false,accessType:'tailscale',sshUser:imageUser,accessMessage:'无公网 IP；请在 Tailscale 管理后台获取 100.x.x.x 地址',sshCommand:`ssh -i <private-key> ${imageUser}@<tailscale-ip>`,cudaProfile:o.expectedCudaMajor===12?'CUDA 12.8':o.allowCuda128Fallback?'优先 CUDA 13.2，允许降级 12.8':'必须 CUDA 13.2',provisioning:'cloud-init-container'};}
...
HyperstackAdapter.prototype.create=async function(options){if(!this.env.TAILSCALE_AUTH_KEY)return hyperstackCreate.call(this,options);this.env.FLEET_PROVIDER='hyperstack';this.env.FLEET_INSTANCE_NAME=options.name||'gpu-fleet';this.env.FLEET_TELEMETRY_PUSH_URL=serviceUrl(this.env,'/api/agent/telemetry');return hyperstackCreate.call(this,options)};
