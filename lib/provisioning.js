...
function bootstrapCommand(env, { keepAlive=false, extra={} }={}) {
  ...
  return `${prefix ? `${prefix}; ` : ''}curl -fsSL --retry 5 ${shellQuote(serviceUrl(env,'/provision/bootstrap.sh'))} -o /tmp/gpu-fleet-bootstrap.sh && bash /tmp/gpu-fleet-bootstrap.sh${keepAlive?` && ${keepAliveCommand}`:''}`;
}
...
function validateProvisioning(provider, env) {
  ...
  if(provider==='hyperstack'){
    ...
    if(!env.TAILSCALE_AUTH_KEY)errors.push('TAILSCALE_AUTH_KEY');
  }
  return errors;
}
