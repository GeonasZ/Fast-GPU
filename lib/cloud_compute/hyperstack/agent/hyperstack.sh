...
for key in FLEET_AGENT_ID FLEET_AGENT_SECRET BASE_URL FLEET_AGENT_BUNDLE_URL FLEET_PROVIDER FLEET_INSTANCE_NAME FLEET_TELEMETRY_PUSH_URL FLEET_STARTUP_SCRIPT_B64; do
  [[ -n "${!key:-}" ]] && env_args+=(-e "$key=${!key}")
done
if [[ -n "${FLEET_STORAGE_ENV_B64:-}" ]]; then
  while IFS= read -r pair; do
    [[ -n "$pair" ]] && env_args+=(-e "$pair")
  done < <(python3 - "$FLEET_STORAGE_ENV_B64" <<'PY'
import base64, json, sys
for key, value in json.loads(base64.b64decode(sys.argv[1])).items():
    print(f"{key}={value}")
PY
  )
fi
...
PY
  ...
  [[ "$(docker inspect -f '{{.State.Running}}' fast-gpu-runtime 2>/dev/null || true)" == "true" ]] || fail "Runtime container exited during bootstrap"
  sleep 5
done
