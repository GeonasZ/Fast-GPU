...
# Fast GPU default Docker startup script. CUDA, PyTorch and SSH are system-managed.
...
`;
const BASE_IMAGE_SCRIPT = `#!/usr/bin/env bash
set -Eeuo pipefail

apt-get update
apt-get install -y --no-install-recommends \\
  ca-certificates curl git jq build-essential cmake python3 python3-pip \\
  fio iperf3 nvme-cli rclone rsync tini pciutils iproute2 \\
  libboost-program-options-dev

${DEFAULT_STARTUP_SCRIPT.replace(/^#![^\n]*\nset -Eeuo pipefail\n\n/, '')}
`;
const DEFAULT_VM_STARTUP_SCRIPT = `#!/usr/bin/env bash
set -Eeuo pipefail
...
function presetDefinitions(env = process.env) {
  ...
  return [
    {id:'preset-pytorch-2.11',name:'PyTorch 2.11 / CUDA 13.2 完整环境',profileType:'docker',image:`${repository}:pytorch-2.11-cuda13.2-ngc26.03`,cudaMajor:13,kind:'complete',recommended:true,script:DEFAULT_STARTUP_SCRIPT},
    {id:'preset-pytorch-2.10',name:'PyTorch 2.10 / CUDA 13.1 完整环境',profileType:'docker',image:`${repository}:pytorch-2.10-cuda13.1-ngc26.01`,cudaMajor:13,kind:'complete',script:DEFAULT_STARTUP_SCRIPT},
    {id:'preset-pytorch-2.7',name:'PyTorch 2.7 / CUDA 12.8 完整环境',profileType:'docker',image:`${repository}:pytorch-2.7-cuda12.8-ngc25.03`,cudaMajor:12,kind:'complete',script:DEFAULT_STARTUP_SCRIPT},
    {id:'preset-ngc-25.01',name:'NGC 25.01 基础环境 + 开发工具',profileType:'docker',image:'nvcr.io/nvidia/pytorch:25.01-py3',cudaMajor:12,kind:'base',script:BASE_IMAGE_SCRIPT},
    {id:'preset-ngc-24.10',name:'NGC 24.10 基础环境 + 开发工具',profileType:'docker',image:'nvcr.io/nvidia/pytorch:24.10-py3',cudaMajor:12,kind:'base',script:BASE_IMAGE_SCRIPT},
    {id:'preset-vm-default',name:'VM 默认开机行为',profileType:'vm',image:'',cudaMajor:13,kind:'vm',recommended:true,script:DEFAULT_VM_STARTUP_SCRIPT},
  ];
}
...
function createImageProfileStore(env = process.env) {
  ...
  db.prepare("UPDATE image_profiles SET startup_script=?,updated_at=? WHERE id='preset-vm-default' AND is_preset=1 AND length(startup_script)<300")
    .run(DEFAULT_VM_STARTUP_SCRIPT,now);
  const row = record => {
