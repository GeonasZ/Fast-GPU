...
const systemToolsDirectory=path.resolve(process.env.FLEET_SYSTEM_TOOLS_DIR||(
  process.platform==='win32'
    ...
    :path.join(process.env.XDG_DATA_HOME||path.join(os.homedir(),'.local','share'),'fast-gpu','tools')
));
...
async function download(url,filename,expectedSha256=''){
  const response=await fetch(url,{headers:{'user-agent':'Fast GPU'}});
...
async function installSsh(scope){
  ...
  const releaseResponse=await fetch('https://api.github.com/repos/PowerShell/Win32-OpenSSH/releases/latest',{headers:{accept:'application/vnd.github+json','user-agent':'Fast GPU'}});
