...
function createClient(config) {
  ...
  client = new S3Client({
    ...
    // S3-compatible providers use the SDK's virtual-hosted style by default.
...
async function listObjects(client, bucket, options = {}) {
  const maxKeys = Math.min(Math.max(Number(options.maxKeys) || 100, 1), 500);
  const result = await client.send(
...
async function deleteObjects(client, bucket, keys) {
  const objects = [...new Set((keys || []).map(String).filter(Boolean))];
  if (!objects.length) return { deleted: [], errors: [] };
  if (objects.length > 1000)
    throw Object.assign(Error("单次最多删除 1000 个对象"), { status: 400 });
  const result = await client.send(
  ...
  return {
    deleted: (result.Deleted || []).map((item) => item.Key).filter(Boolean),
...
async function probeAccess(config, client = createClient(config)) {
  const token = randomBytes(16),
    key = `${String(config.prefix || "").replace(/^\/+|\/+$/g, "")}${config.prefix ? "/" : ""}.fast-gpu-probe-${randomBytes(8).toString("hex")}`,
    result = {
