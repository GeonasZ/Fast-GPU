...
function createProviderKeyStore(env = process.env) {
  ...
  const listEntries = db.prepare('SELECT id, provider, key_suffix AS keySuffix, created_at AS createdAt FROM provider_api_key_entries WHERE provider = ? ORDER BY rowid DESC');
  const insertEntry = db.prepare('INSERT INTO provider_api_key_entries (id, provider, encrypted_key, encryption_iv, encryption_tag, encryption_algorithm, key_suffix, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
  ...
  function aad(provider) { return Buffer.from(`gpu-fleet:provider-api-key:${provider}`); }
  function entryAad(provider, id) { return Buffer.from(`gpu-fleet:provider-api-key:${provider}:${id}`); }
  ...
  function availableKeys(provider) {
    ...
    if (legacy) entries.push({ id: `legacy:${provider}`, provider, keySuffix: legacy.key_suffix, createdAt: legacy.created_at });
    return entries;
  }
  ...
  return {
    ...
    add(provider, value) {
      ...
      const encrypted = Buffer.concat([cipher.update(apiKey, 'utf8'), cipher.final()]);
      insertEntry.run(id, provider, encrypted, iv, cipher.getAuthTag(), ALGORITHM, apiKey.slice(-4), new Date().toISOString());
      setActive.run(provider, id, new Date().toISOString());
      return { id, provider, keySuffix: apiKey.slice(-4), createdAt: new Date().toISOString() };
    },
    get(provider) {
